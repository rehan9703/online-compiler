...
LANGUAGES = {
    ...
    'typescript': {
        ...
        'extension': 'ts',
        'kind': 'interpreted',
        'run_cmd': ['npx.cmd' if os.name == 'nt' else 'npx', '--yes', 'ts-node'],
        'version_cmd': ['npx.cmd' if os.name == 'nt' else 'npx', '--yes', 'ts-node', '--version'],  # fast check — tsx itself downloads on first run
    },
    'batch': {
        'display': 'Batch',
        'extension': 'bat',
